...
public class Bloque {
	...
	// Timestamp de creaci�n del bloque
	...
	/**
	 * Calcular el hash del bloque a partir de la informaci�n de la cabecera del bloque (sin transacciones)
	 * 
	 * @return Hash SHA256
	 */
	...
	public byte[] calcularRaizArbolMerkle() {
		Queue<byte[]> colaHashes = new LinkedList<>(
				 transacciones.stream().map(Transaccion::getHash).collect(Collectors.toList())); //map realizar transformacion sobre un stream 
		while (colaHashes.size() > 1) {															// collector, convertir todo a una lista
	...
	public boolean esValido() { // REALIZAR TODAVIA !!!!!
		if(this.hash == null) {
